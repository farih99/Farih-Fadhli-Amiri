...
1. **Pemantauan manual berskala besar**
   Guru tidak perlu memeriksa data siswa satu per satu hanya untuk mencari siswa yang mulai berisiko.

2. **Keterlambatan identifikasi masalah**
   Sistem membantu memberikan indikasi lebih awal berdasarkan pola data yang tersedia.

3. **Kesulitan membaca data mentah**
   Data absensi dan nilai diterjemahkan menjadi informasi risiko yang lebih mudah dipahami.

4. **Sulit menentukan prioritas intervensi**
   Guru dapat memusatkan perhatian pada siswa dengan risiko lebih tinggi terlebih dahulu.

### Gain yang Diciptakan

Produk diharapkan memberikan:

* **Visibilitas risiko:** guru dapat mengetahui siswa yang perlu mendapatkan perhatian.
* **Prioritas tindakan:** skor risiko membantu menentukan siswa yang lebih mendesak untuk ditindaklanjuti.
* **Keputusan berbasis data:** intervensi tidak hanya mengandalkan pengamatan subjektif.
* **Efisiensi pemantauan:** informasi penting dirangkum dalam bentuk yang lebih mudah digunakan.
* **Intervensi preventif:** guru memperoleh kesempatan melakukan pembinaan sebelum masalah kedisiplinan berkembang lebih jauh.

### Mengapa AI Bukan Gimmick

Fitur AI memiliki fungsi substantif karena problem yang dihadapi bukan sekadar penyajian data, melainkan **identifikasi pola risiko dari sejumlah besar data siswa**.

Empat variabel utama, yaitu **persentase absensi, jumlah alfa, keterlambatan, dan prestasi akademik**, telah teridentifikasi sebagai fitur yang berpengaruh terhadap kategori kedisiplinan. Pengujian terhadap dataset 711 siswa juga menunjukkan bahwa pendekatan klasifikasi mampu menghasilkan prediksi dengan akurasi hingga **77,62%**.

Dengan demikian, AI digunakan untuk menjawab kebutuhan inti produk, yaitu **membantu mendeteksi siswa yang berpotensi mengalami penurunan kedisiplinan**, bukan sekadar menambahkan label AI pada sistem administrasi.
...
**Catatan:** Target KPI penggunaan di atas merupakan **[ASUMSI-06]** karena konteks belum menyediakan baseline penggunaan atau target resmi dari sekolah.
...
| ----------------------------------------------------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| **[ASUMSI-01] Data absensi dan nilai dapat diakses pengguna**           | Sistem tidak dapat digunakan secara optimal                | Menetapkan kebutuhan data minimum sejak awal pengembangan                               |
| **Data hanya berasal dari satu sekolah**                                | Generalisasi model ke sekolah lain terbatas                | Menyatakan batasan validitas model dan tidak mengklaim hasil berlaku universal          |
| **Akurasi model sekitar 77,62% belum berarti semua prediksi benar**     | Risiko salah klasifikasi siswa                             | Menampilkan hasil sebagai **indikasi risiko**, bukan keputusan final                    |
| **Data historis terbatas**                                              | Model dapat kurang merepresentasikan variasi kondisi siswa | Menggunakan data yang tersedia secara konsisten dan mencatat keterbatasan dataset       |
| **Guru memiliki banyak siswa**                                          | Informasi terlalu banyak dapat tetap membebani pengguna    | Memprioritaskan daftar siswa berisiko dan ringkasan informasi                           |
| **Pengguna tidak terbiasa menggunakan sistem berbasis AI**              | Adopsi produk rendah                                       | Menyediakan tampilan sederhana dan hasil prediksi yang mudah dipahami                   |
| **Rekomendasi intervensi dapat disalahartikan sebagai keputusan final** | Potensi tindakan yang kurang tepat                         | Memberikan rekomendasi sebagai bahan pertimbangan guru                                  |
| **Integrasi dengan sistem existing kompleks**                           | Waktu pengembangan prototype terlampaui                    | Menjadikan integrasi sebagai scope minimal dan bukan fokus utama                        |
| **Waktu prototype hanya satu semester**                                 | Fitur terlalu banyak dapat mengurangi kualitas fitur inti  | Memprioritaskan fitur Must Have, terutama monitoring dan AI early-warning               |
| **Model ringan tetap memiliki keterbatasan prediksi**                   | Hasil tidak selalu akurat pada seluruh siswa               | Melakukan evaluasi menggunakan metrik klasifikasi dan mendokumentasikan keterbatasannya |

## Fokus Produk 3 Bulan

Secara praktis, MVP DisiplinGuard sebaiknya berpusat pada satu alur utama:

**Data siswa → indikator absensi & prestasi → ★ prediksi tingkat kedisiplinan → ★ skor risiko → ★ rekomendasi intervensi → guru menentukan tindak lanjut.**

Dengan batas tersebut, DisiplinGuard tetap fokus pada problem nyata yang ingin diselesaikan, yaitu **mengubah data administratif sekolah menjadi sinyal peringatan dini yang dapat membantu guru bertindak lebih cepat**.
